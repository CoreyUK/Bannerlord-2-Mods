...
internal static class StrategicAiHelpers
{
    ...
    public static float NearbyFriendlyLordStrength(Settlement settlement, IFaction faction, float radius)
    {
        ...
        foreach (MobileParty party in StrategicAiCache.GetActiveLordParties())
        {
            if (IsFriendly(faction, party.MapFaction) && Distance(party, settlement) <= radius)
            {
                total += party.GetTotalLandStrengthWithFollowers(false);
            }
        }
    ...
    public static Settlement? FindBestRetreatSettlement(MobileParty party)
    {
        ...
        foreach (Settlement settlement in StrategicAiCache.GetFortifications())
        {
            if (!IsOwnTerritory(faction, settlement) || settlement.IsUnderSiege)
            {
                continue;
            }

            float distance = Distance(party, settlement);
            float enemyThreat = NearbyEnemyLordStrength(settlement, faction!, StrategicAiTuning.HomelandDefenseThreatRadius);
            float friendlySupport = NearbyFriendlyLordStrength(settlement, faction!, StrategicAiTuning.LocalAllyRadius);
    ...
    /// <summary>
    /// Enemy lord parties near a party. The previous implementation filtered
    /// MobileParty.AllPartiesWithoutPartyComponent by IsLordParty -- lord parties
    /// always have a LordPartyComponent, so that list was always empty and every
    /// caller silently saw zero strength.
    /// </summary>
    public static IEnumerable<MobileParty> EnemyLordPartiesNear(MobileParty party, float radius)
    {
        foreach (MobileParty other in StrategicAiCache.GetActiveLordParties())
        {
            if (other != party && IsEnemy(party.MapFaction, other.MapFaction) && Distance(party, other) <= radius)
            {
                yield return other;
            }
        }
    ...
    public static IEnumerable<MobileParty> FriendlyLordPartiesNear(MobileParty party, float radius)
    {
        foreach (MobileParty other in StrategicAiCache.GetActiveLordParties())
        {
            if (other != party && IsFriendly(party.MapFaction, other.MapFaction) && Distance(party, other) <= radius)
            {
                yield return other;
            }
        }
